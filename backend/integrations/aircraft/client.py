...
from .repository import AircraftTypeRepository
...
_aircraft_type_repository = AircraftTypeRepository()
...
def _lookup_type_name(iata_code: str | None) -> str | None:
    ...
    if not iata_code:
        return None
    return _aircraft_type_repository.get_name(iata_code)
...
async def _fetch_type_name_from_hexdb(icao24: str) -> tuple[str, str, str]:
    ...
    try:
        ...
        if iata_code:
            existing_name = _aircraft_type_repository.get_name(iata_code)
            if existing_name:
                return existing_name, iata_code, registration
        ...
        if iata_code and name:
            _aircraft_type_repository.cache(iata_code, name, manufacturer)
...
async def get_or_fetch_aircraft(flight_id: str, flight_number: str, user_id: int) -> dict:
    """Return cached aircraft info, or fetch live if not yet cached."""
    from ...flights.repository import FlightRepository

    repository = FlightRepository()
    row = repository.get_aircraft_fields(flight_id)
    ...
    repository.update(
        ...
        user_id,
        {
            "aircraft_type": info.get("type_name") or "",
            "aircraft_icao": info.get("icao24") or "",
            "aircraft_registration": info.get("registration") or "",
            "aircraft_fetched_at": now,
            "updated_at": now,
        },
    )
