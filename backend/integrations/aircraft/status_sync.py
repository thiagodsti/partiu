...
def _maybe_send_alert(row: sqlite3.Row, status_info: dict) -> None:
    ...
    if new_status == "cancelled" and prev_status != "cancelled":
        if not push_service.already_sent(user_id, flight_id, "delay_alert_cancelled"):
            payload = {
                "title": f"Flight {flight_number} cancelled",
                "body": f"{dep} → {arr}",
                "url": url,
            }
            if push_service.send_push(user_id, payload):
                push_service.log_sent(user_id, flight_id, "delay_alert_cancelled")
                logger.info(
    ...
    if new_status == "diverted" and prev_status != "diverted":
        if not push_service.already_sent(user_id, flight_id, "delay_alert_diverted"):
            payload = {
            ...
            if push_service.send_push(user_id, payload):
                push_service.log_sent(user_id, flight_id, "delay_alert_diverted")
                logger.info(
    ...
    if new_delay >= _DELAY_ALERT_THRESHOLD and prev_delay < _DELAY_ALERT_THRESHOLD:
        if not push_service.already_sent(user_id, flight_id, "delay_alert_delay"):
            payload = {
            ...
            if push_service.send_push(user_id, payload):
                push_service.log_sent(user_id, flight_id, "delay_alert_delay")
