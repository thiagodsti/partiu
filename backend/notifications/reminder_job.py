    ...
    if notif_flight or notif_checkin:
        ...
        for row in flights:
            ...
            if notif_flight and _FLIGHT_WINDOW[0] <= time_until <= _FLIGHT_WINDOW[1]:
                fid = str(row["id"])
                if not push_service.already_sent(user_id, fid, "flight_reminder"):
                    ...
                    sent = push_service.send_push(user_id, {"title": title, "body": body, "url": url})
                    if sent:
                        notification_service.create_notification(user_id, "flight_reminder", title, body, url)
                        push_service.log_sent(user_id, fid, "flight_reminder")
            ...
            if notif_checkin and _CHECKIN_WINDOW[0] <= time_until <= _CHECKIN_WINDOW[1]:
                fid = str(row["id"])
                if not push_service.already_sent(user_id, fid, "checkin_reminder"):
                    ...
                    sent = push_service.send_push(user_id, {"title": title, "body": body, "url": url})
                    if sent:
                        notification_service.create_notification(user_id, "checkin_reminder", title, body, url)
                        push_service.log_sent(user_id, fid, "checkin_reminder")
    ...
    if notif_trip:
        ...
        for row in trips:
            ...
            if _TRIP_WINDOW[0] <= time_until <= _TRIP_WINDOW[1]:
                tid = str(row["id"])
                if not push_service.already_sent(user_id, tid, "trip_reminder"):
                    title = t("notif.trip_reminder_title", locale, name=row["name"])
                    body = t("notif.trip_reminder_body", locale)
                    url = f"/#/trips/{tid}"
                    sent = push_service.send_push(user_id, {"title": title, "body": body, "url": url})
                    if sent:
                        notification_service.create_notification(user_id, "trip_reminder", title, body, url)
                        push_service.log_sent(user_id, tid, "trip_reminder")
