...
def _extract_flight_number(
    text: str, airline_codes: list[str] | None = None
) -> str | None:
        ...
        if len(segment_matches) >= 2 and flight_nums:
            ...
            if connections:
                ...
                segments.append(
                    {
                        ...
                        "flight_number": (
                            segments[-1]["next_flight"] if segments else first_flight
                        ),
                        "layover_minutes": 0,
                    }
                )
                ...
                if len(segment_matches) >= 2 * n_segments:
                    ...
                    distances = [
                        _airport_distance(s["dep_airport"], s["arr_airport"])
                        for s in segments
                    ]
                ...
                for i in range(0, len(segment_matches) - 1, 2):
                    ...
                    fn_idx = i // 2
                    fn = (
                        flight_nums[fn_idx].strip() if fn_idx < len(flight_nums) else ""
                    )
    ...
    for i, date_m in enumerate(date_matches):
        ...
        if arr_dt and dep_dt and arr_dt < dep_dt:
            arr_dt = _build_datetime(
                date_type(dep_date.year, dep_date.month, dep_date.day)
                + timedelta(days=1),
                arr_time,
            )
