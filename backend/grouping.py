...
Groups flights by booking reference and time proximity.
...
def auto_group_flights(
    ...
    if not ungrouped:
        ...
        return {
            ...
            "message": "No ungrouped flights found"
            + (f", merged {merged} groups" if merged else ""),
        }
...
def _build_trip_name(flights: list[dict], booking_ref: str = "") -> str:
    ...
    if origin_city and dest_city and origin_city != dest_city:
        name = (
            f"{origin_city} → {dest_city} ({date_str})"
            if date_str
            else f"{origin_city} → {dest_city}"
        )
    elif dest_city:
...
def _create_trip_for_flights(
    ...
    if trip_id:
        ...
        try:
            ...
            logger.error("Error updating trip %s: %s", trip_id, e)
            return None
...
def _merge_overlapping_groups(max_gap: timedelta, user_id: int | None = None) -> int:
    ...
    while changed:
        ...
        for i, g1 in enumerate(trip_list):
            ...
            g1_end = _dt_from_iso(
                g1_flights[-1].get("arrival_datetime", "")
                or g1_flights[-1].get("departure_datetime", "")
            )
            ...
            for g2 in trip_list[i + 1 :]:
                ...
                g2_end = _dt_from_iso(
                    g2_flights[-1].get("arrival_datetime", "")
                    or g2_flights[-1].get("departure_datetime", "")
                )
                ...
                overlap = g1_start <= g2_end + max_gap and g2_start <= g1_end + max_gap
                if overlap:
                    # Merge g2 into g1
                    now = now_iso()
                    all_flights = sorted(
                        g1_flights + g2_flights, key=lambda f: f.get("departure_datetime", "")
                    )
                    refs = list(
                        {
                            f.get("booking_reference", "")
                    ...
                    new_end = (
                        last_f.get("arrival_datetime", "")
                        or last_f.get("departure_datetime", "")
                        or ""
                    )[:10]
