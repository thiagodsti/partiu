"""Unit tests for backend.auth.session (cross-cutting session/password helpers)."""
...
class TestSessionCookies:
    def test_create_and_decode_session(self, test_db):
        from backend.auth import hash_password
        from backend.auth.session import create_session_cookie, decode_session_cookie
    ...
    def test_decode_invalid_token(self, test_db):
        from backend.auth.session import decode_session_cookie
    ...
    def test_revoke_invalid_token_no_error(self, test_db):
        from backend.auth.session import revoke_session_cookie
...
class TestPending2FAToken:
    def test_create_and_decode(self, test_db):
        from backend.auth.session import create_pending_2fa_cookie, decode_pending_2fa_cookie

        token = create_pending_2fa_cookie(42)
        assert decode_pending_2fa_cookie(token) == 42

    def test_decode_invalid(self, test_db):
        from backend.auth.session import decode_pending_2fa_cookie
