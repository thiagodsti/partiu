...
Fixture: tests/fixtures/sas_eticket_pdf_anonymized.eml
  CPT→CDG AF871  |  CDG→ARN AF1462
...
def sas_scandinavian_airlines_email():
    return load_eml_as_email_message("sas_eticket_pdf_anonymized.eml")
...
class TestSASScandinavianAirlinesLegOne:
    def test_flight_number(self, sas_scandinavian_airlines_flights):
        assert sas_scandinavian_airlines_flights[0]["flight_number"] == "AF871"

    def test_departure_airport(self, sas_scandinavian_airlines_flights):
        assert sas_scandinavian_airlines_flights[0]["departure_airport"] == "CPT"

    def test_arrival_airport(self, sas_scandinavian_airlines_flights):
        assert sas_scandinavian_airlines_flights[0]["arrival_airport"] == "CDG"
    ...
    def test_departure_datetime(self, sas_scandinavian_airlines_flights):
        assert sas_scandinavian_airlines_flights[0]["departure_datetime"] == dt(2026, 11, 12, 7, 55)

    def test_arrival_datetime(self, sas_scandinavian_airlines_flights):
        assert sas_scandinavian_airlines_flights[0]["arrival_datetime"] == dt(2026, 11, 12, 19, 15)


# ---------------------------------------------------------------------------
# Leg 2: CDG → ARN  AF1462
# ---------------------------------------------------------------------------


class TestSASScandinavianAirlinesLegTwo:
    def test_flight_number(self, sas_scandinavian_airlines_flights):
        assert sas_scandinavian_airlines_flights[1]["flight_number"] == "AF1462"

    def test_departure_airport(self, sas_scandinavian_airlines_flights):
        assert sas_scandinavian_airlines_flights[1]["departure_airport"] == "CDG"

    def test_arrival_airport(self, sas_scandinavian_airlines_flights):
        assert sas_scandinavian_airlines_flights[1]["arrival_airport"] == "ARN"

    def test_airline_code(self, sas_scandinavian_airlines_flights):
        assert sas_scandinavian_airlines_flights[1]["airline_code"] == "SK"

    def test_departure_datetime(self, sas_scandinavian_airlines_flights):
        assert sas_scandinavian_airlines_flights[1]["departure_datetime"] == dt(2026, 11, 12, 21, 0)

    def test_arrival_datetime(self, sas_scandinavian_airlines_flights):
        assert sas_scandinavian_airlines_flights[1]["arrival_datetime"] == dt(2026, 11, 12, 23, 40)
