"""Tests for backend.integrations.aircraft.sync."""
...
class TestRunAircraftSync:
    def test_returns_dict_with_expected_keys(self, test_db):
        from backend.integrations.aircraft.sync import run_aircraft_sync
    ...
    def test_cancelled_flights_are_skipped(self, test_db):
        ...
        with patch(
            "backend.integrations.aircraft.sync._fetch_rows",
...
class TestFetchAircraftForNewFlights:
    def test_empty_list_is_noop(self, test_db):
        from backend.integrations.aircraft.sync import fetch_aircraft_for_new_flights
...
class TestFetchRows:
    def test_gives_up_on_old_arrivals(self, test_db):
        ...
        from backend.integrations.aircraft.sync import _fetch_rows
...
class TestUnconfirmedRefresh:
    def test_unconfirmed_flight_within_24h_is_refetched(self, test_db):
        ...
        with patch(
            "backend.integrations.aircraft.client.fetch_aircraft_info", new=AsyncMock(return_value=info)
        ):
            result = run_aircraft_sync()
    ...
    def test_confirmed_flight_is_not_refetched(self, test_db):
        ...
        with patch("backend.integrations.aircraft.client.fetch_aircraft_info", new=mock_fetch):
            run_aircraft_sync()
...
class TestRecoverMissingAircraftNames:
    def test_no_eligible_flights_returns_zero(self, test_db):
        from backend.integrations.aircraft.sync import _recover_missing_aircraft_names
