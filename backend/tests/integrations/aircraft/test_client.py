"""Tests for backend.integrations.aircraft.client."""
...
class TestLookupTypeName:
    def test_empty_string_returns_none(self, test_db):
        from backend.integrations.aircraft.client import _lookup_type_name
...
class TestFetchTypeNameFromHexdb:
    def test_empty_icao24_returns_empty(self):
        from backend.integrations.aircraft.client import _fetch_type_name_from_hexdb
...
class TestResolveAircraftName:
    def test_local_table_hit_returns_name(self, test_db):
        from backend.database import db_write
        from backend.integrations.aircraft.client import resolve_aircraft_name
...
class TestFetchViaAviationStack:
    def test_success(self):
        from backend.integrations.aircraft.client import _fetch_via_aviationstack
...
class TestFetchViaOpensky:
    def test_success(self):
        from backend.integrations.aircraft.client import _fetch_via_opensky
...
class TestFetchAircraftInfo:
    def test_uses_aviationstack_when_key_configured(self, test_db, monkeypatch):
        ...
        from backend.integrations.aircraft.client import fetch_aircraft_info

        monkeypatch.setattr(cfg.settings, "AVIATIONSTACK_API_KEY", "test_key")
        raw = {"icao24": "ABC123", "iata_code": "B738", "registration": "N123"}
        with patch(
            "backend.integrations.aircraft.client._fetch_via_aviationstack", new=AsyncMock(return_value=raw)
        ):
            with patch(
                "backend.integrations.aircraft.client.resolve_aircraft_name",
    ...
    def test_falls_back_to_opensky_when_no_key(self, test_db, monkeypatch):
        ...
        with patch("backend.integrations.aircraft.client._fetch_via_opensky", new=AsyncMock(return_value=raw)):
            with patch(
                "backend.integrations.aircraft.client.resolve_aircraft_name",
    ...
    def test_no_data_returns_empty_dict(self, test_db, monkeypatch):
        ...
        monkeypatch.setattr(cfg.settings, "AVIATIONSTACK_API_KEY", "")
        with patch("backend.integrations.aircraft.client._fetch_via_opensky", new=AsyncMock(return_value={})):
    ...
    def test_aviationstack_empty_falls_back_to_opensky(self, test_db, monkeypatch):
        ...
        with patch("backend.integrations.aircraft.client._fetch_via_aviationstack", new=AsyncMock(return_value={})):
            with patch("backend.integrations.aircraft.client._fetch_via_opensky", new=AsyncMock(return_value=raw)):
                with patch(
                    "backend.integrations.aircraft.client.resolve_aircraft_name",
...
class TestGetOrFetchAircraft:
    ...
    def _seed_user(self, conn) -> int:
        cur = conn.execute(
            "INSERT INTO users (username, password_hash, is_admin) VALUES ('u', 'h', 0)"
        )
    ...
    def _flight_insert(
        ...
        conn.execute(
            """INSERT INTO flights (id, flight_number, departure_airport, arrival_airport,
               departure_datetime, arrival_datetime, aircraft_type, aircraft_icao,
               aircraft_fetched_at, user_id, created_at, updated_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                flight_id,
                flight_number,
    ...
    def test_returns_cached_data(self, test_db):
        from backend.database import db_write
        from backend.integrations.aircraft.client import get_or_fetch_aircraft

        flight_id = str(uuid.uuid4())
        with db_write() as conn:
            user_id = self._seed_user(conn)
            conn.execute(
                """INSERT INTO flights (id, flight_number, departure_airport, arrival_airport,
                   departure_datetime, arrival_datetime, aircraft_type, aircraft_icao,
                   aircraft_registration, aircraft_fetched_at, user_id, created_at, updated_at)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (
                    flight_id,
                    "LA100",
        ...
        result = asyncio.run(get_or_fetch_aircraft(flight_id, "LA100", user_id))
    ...
    def test_fetches_and_stores_when_not_cached(self, test_db):
        ...
        with db_write() as conn:
            user_id = self._seed_user(conn)
            self._flight_insert(conn, flight_id, "LA200", user_id)
        info = {
            "icao24": "XYZ789",
            "iata_code": "B738",
        ...
        with patch("backend.integrations.aircraft.client.fetch_aircraft_info", new=AsyncMock(return_value=info)):
            result = asyncio.run(get_or_fetch_aircraft(flight_id, "LA200", user_id))
    ...
    def test_empty_info_stores_empty_values(self, test_db):
        ...
        with db_write() as conn:
            user_id = self._seed_user(conn)
            self._flight_insert(conn, flight_id, "LA300", user_id)
        with patch("backend.integrations.aircraft.client.fetch_aircraft_info", new=AsyncMock(return_value={})):
            result = asyncio.run(get_or_fetch_aircraft(flight_id, "LA300", user_id))
