"""Tests for backend.integrations.aircraft.status_sync."""
...
class TestRunFlightStatusSync:
    def test_skips_when_no_api_key(self, test_db, monkeypatch):
        ...
        from backend.integrations.aircraft.status_sync import run_flight_status_sync
    ...
    def test_upcoming_flight_in_window_is_checked(self, test_db, monkeypatch):
        ...
        with (
            patch(
                "backend.integrations.aircraft.status_sync._fetch_status_from_aviationstack",
                new=AsyncMock(return_value=mock_status),
            ),
            patch("backend.integrations.aircraft.status_sync._maybe_send_alert"),
        ):
            result = run_flight_status_sync()
...
class TestMaybeSendAlert:
    ...
    def test_sends_cancelled_alert_on_first_cancellation(self, test_db):
        from backend.integrations.aircraft.status_sync import _maybe_send_alert

        row = self._make_row(prev_status=None)
        status_info = {"flight_status": "cancelled", "departure_delay": None}

        with (
            patch("backend.notifications.push_service.push_service.send_push", return_value=1) as mock_push,
            patch("backend.notifications.push_service.push_service.already_sent", return_value=False),
            patch("backend.notifications.push_service.push_service.log_sent"),
        ):
    ...
    def test_does_not_resend_cancelled_alert(self, test_db):
        ...
        with (
            patch("backend.notifications.push_service.push_service.send_push") as mock_push,
            patch("backend.notifications.push_service.push_service.already_sent", return_value=False),
        ):
            _maybe_send_alert(row, status_info)
...
class TestFetchStatusFromAviationstack:
    def test_returns_empty_on_http_error(self):
        ...
        from backend.integrations.aircraft.status_sync import _fetch_status_from_aviationstack

        mock_resp = MagicMock()
        mock_resp.status_code = 500

        with patch("backend.integrations.aircraft.status_sync.httpx.AsyncClient") as mock_client_cls:
