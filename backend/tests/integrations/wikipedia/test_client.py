"""Tests for backend.integrations.wikipedia.client."""
...
class TestTripImagePath:
    def test_path_format(self, test_db):
        from backend.integrations.wikipedia.client import trip_image_path
...
class TestParseJsonSafe:
    def test_returns_dict_for_200(self):
        from backend.integrations.wikipedia.client import _parse_json_safe
...
class TestGetPhotoUrls:
    def test_returns_urls_from_generator(self):
        from backend.integrations.wikipedia.client import _get_photo_urls
...
class TestFetchTripImage:
    def test_returns_true_when_file_exists(self, test_db):
        from backend.integrations.wikipedia.client import fetch_trip_image, trip_image_path
    ...
    def test_force_refresh_deletes_existing(self, test_db):
        ...
        with patch("backend.integrations.wikipedia.client._get_photo_urls", AsyncMock(return_value=[])):
    ...
    def test_returns_false_when_no_urls(self, test_db):
        from backend.integrations.wikipedia.client import fetch_trip_image
    ...
    def test_downloads_and_saves_image(self, test_db):
        ...
        with patch(
            "backend.integrations.wikipedia.client._get_photo_urls",
            AsyncMock(return_value=["https://example.com/img.jpg"]),
        ):
            with patch("httpx.AsyncClient", return_value=mock_dl_client):
                with patch("backend.integrations.wikipedia.client._resize_and_encode_webp", return_value=fake_webp):
    ...
    def test_download_exception_tries_next_url(self, test_db):
        ...
        with patch(
            ...
            with patch("httpx.AsyncClient", return_value=mock_client):
                with patch(
                    "backend.integrations.wikipedia.client._resize_and_encode_webp", return_value=b"RIFF fake webp"
                ):
