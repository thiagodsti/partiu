"""Tests for backend.notifications.reminder_job scheduler job."""
...
class TestRunPushNotifications:
    def test_no_error_with_no_users(self, test_db):
        from backend.notifications.reminder_job import run_push_notifications
    ...
    def test_flight_reminder_sent_in_window(self, test_db, monkeypatch):
        ...
        with patch("pywebpush.webpush") as mock_wp:
            ...
            push_service.subscribe(uid, PushSubscription("https://e.com/1", "p", "a"))
            run_push_notifications()

        from backend.notifications.push_service import push_service

        assert push_service.already_sent(uid, fid, "flight_reminder") is True
    ...
    def test_flight_reminder_not_sent_outside_window(self, test_db, monkeypatch):
        ...
        with patch("pywebpush.webpush") as mock_wp:
            ...
            push_service.subscribe(uid, PushSubscription("https://e.com/2", "p", "a"))
            run_push_notifications()

        from backend.notifications.push_service import push_service

        assert push_service.already_sent(uid, fid, "flight_reminder") is False
    ...
    def test_checkin_reminder_sent_in_window(self, test_db, monkeypatch):
        ...
        with patch("pywebpush.webpush") as mock_wp:
            ...
            push_service.subscribe(uid, PushSubscription("https://e.com/3", "p", "a"))
            run_push_notifications()

        from backend.notifications.push_service import push_service

        assert push_service.already_sent(uid, fid, "checkin_reminder") is True
    ...
    def test_trip_reminder_sent_in_window(self, test_db, monkeypatch):
        ...
        with patch("pywebpush.webpush") as mock_wp:
            ...
            push_service.subscribe(uid, PushSubscription("https://e.com/4", "p", "a"))
            run_push_notifications()

        from backend.notifications.push_service import push_service

        assert push_service.already_sent(uid, tid, "trip_reminder") is True
    ...
    def test_disabled_notif_not_sent(self, test_db, monkeypatch):
        ...
        with patch("pywebpush.webpush") as mock_wp:
            ...
            push_service.subscribe(uid, PushSubscription("https://e.com/5", "p", "a"))
            run_push_notifications()

        from backend.notifications.push_service import push_service

        assert push_service.already_sent(uid, fid_disabled, "flight_reminder") is False
    ...
    def test_duplicate_not_sent_twice(self, test_db, monkeypatch):
        ...
        push_service.subscribe(uid, PushSubscription("https://e.com/6", "p", "a"))
