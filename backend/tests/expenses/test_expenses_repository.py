"""Tests for backend.expenses.repository (raw CRUD for the trip_expenses table)."""
...
class TestCreate:
    def test_create_and_list(self, test_db):
        ...
        repo.create(expense_id, trip_id, "Hotel", 500.0, "EUR", user_id)
    ...
    def test_created_by_username_is_joined(self, test_db):
        ...
        repo.create(str(uuid.uuid4()), trip_id, "Hotel", 500.0, "EUR", user_id)
    ...
    def test_isolates_by_trip(self, test_db):
        ...
        repo.create(str(uuid.uuid4()), trip1, "For trip 1", 10.0, "EUR", user_id)
...
class TestExists:
    def test_true_for_existing(self, test_db):
        ...
        trip_id = _seed_trip(test_db, user_id)
        expense_id = str(uuid.uuid4())
        repo.create(expense_id, trip_id, "Hotel", 10.0, "EUR", user_id)
...
class TestUpdate:
    def test_update_fields_and_bumps_updated_at(self, test_db):
        ...
        trip_id = _seed_trip(test_db, user_id)
        expense_id = str(uuid.uuid4())
        repo.create(expense_id, trip_id, "Old", 10.0, "EUR", user_id)
