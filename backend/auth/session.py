"""
Cross-cutting authentication helpers: password hashing, session cookies, the
``get_current_user``/``require_admin`` FastAPI dependencies used by every route in
the app. Re-exported from ``backend.auth`` (the package __init__) so the rest of the
backend can keep importing ``from ..auth import get_current_user`` etc. unchanged.

The actual auth *feature* (setup/login/2FA/logout/me/change-password endpoints) is
routes.py/service.py/repository.py in this same package — it imports these helpers
as a sibling module (``from .session import ...``).
"""
