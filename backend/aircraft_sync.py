...
async def _recover_missing_aircraft_names() -> int:
    ...
    for row in rows:
        ...
        if type_name or registration:
            ...
            recovered += 1
            logger.info('Aircraft recovery: %s → %s / %s', row['aircraft_icao'], type_name, registration)
...
async def _fetch_rows(rows) -> dict:
    ...
    for row in rows:
        ...
        if arr_str:
            try:
                ...
                if now - arr > _GIVE_UP_AFTER:
                    ...
                    given_up += 1
                    logger.debug('Aircraft sync: giving up on %s (>24h past arrival)', row['flight_number'])
                    continue
            except ValueError:
                pass
        ...
        if info.get('icao24') or info.get('type_name'):
            ...
            updated += 1
            logger.info('Aircraft sync: %s → %s (%s)',
                        row['flight_number'], info.get('type_name') or '?', info.get('icao24') or '?')
        else:
            logger.debug('Aircraft sync: no data yet for %s — will retry', row['flight_number'])
